{
  keymaps = [
    {
      key = "<leader>q";
      action = "<cmd>lua do if #vim.api.nvim_tabpage_list_wins(0) > 1 then vim.cmd.close() else vim.cmd.bd() end end<cr>";
    ...
    {
      key = "<c-n>";
      action = "<cmd>lua if not pcall(vim.cmd.lnext) then vim.cmd.lfirst() end<cr>";
      options = { desc = "Select next item in location list"; };
    }
    {
      key = "<c-p>";
      action = "<cmd>lua if not pcall(vim.cmd.lnext) then vim.cmd.llast() end<cr>";
